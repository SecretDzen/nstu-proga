...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST["position"]) && isset($_POST["degree"]) && isset($_POST["course"]) && isset($_POST["surname"]) && isset($_POST["room"])) {
        ...
        $degree_id = pg_fetch_all(pg_query_params($db, 'SELECT id from labs.degree where name = $1 limit 1;', array($_POST['degree'])), PGSQL_ASSOC)[0]['id'];
        $course_id = pg_fetch_all(pg_query_params($db, 'SELECT id from labs.courses where name = $1 limit 1;', array($_POST['course'])), PGSQL_ASSOC)[0]['id'];

        if (!$position_id || $position_id == 0) {
            pg_query_params($db, 'INSERT into labs."position" (name) values ($1);', array($_POST['position']));
            $position_id = pg_fetch_all(pg_query_params($db, 'select id from labs."position" where name = $1 limit 1;', array($_POST['position'])), PGSQL_ASSOC)[0];
        }

        if (!$degree_id || $degree_id == 0) {
            pg_query_params($db, 'INSERT into labs.degree (name) values ($1);', array($_POST['degree']));
            $degree_id = pg_fetch_all(pg_query_params($db, 'select id from labs.degree where name = $1 limit 1;', array($_POST['degree'])), PGSQL_ASSOC)[0];
        }

        if (!$course_id || $course_id == 0) {
            pg_query_params($db, 'INSERT into labs.courses (name) values ($1);', array($_POST['course']));
            $course_id = pg_fetch_all(pg_query_params($db, 'select id from labs.courses where name = $1 limit 1;', array($_POST['course'])), PGSQL_ASSOC)[0];
        }


        $result = pg_query_params(
            $db,
            'UPDATE labs.teachers set (position, degree, courses, surname, room_number) = ($1, $2, $3, $4, $5) where id = $6;',
